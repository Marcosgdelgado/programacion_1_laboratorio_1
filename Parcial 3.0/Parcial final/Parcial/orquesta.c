...
int orquesta_alta(Orquesta array[], int size, int* contadorID)                          //cambiar Orquesta
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            utn_getTexto("\nIngrese direccion:\n","Error",1,TEXT_SIZE,3,array[posicion].varLongString);
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            {       //copiar printf de alta
                printf("\n Posicion: %d\n ID: %d\n varInt: %d\n Nombre: %s\n Tipo: %s",
                       posicion, array[posicion].idOrquesta,array[posicion].varInt,array[posicion].varString,array[posicion].varLongString);
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n Nombre: %s\nDireccion: %s\n",
                       array[i].idOrquesta,array[i].varString,array[i].varLongString);
                       auxtipo= array[i].tipoOrquesta;
            switch(auxtipo)
            {
                case 1:
                printf("Sinfonia\n");
...
void orquesta_hardcodearDatos(Orquesta array[], int size, int *contadorID)
{
    ...
    char lugar[][20]={"Lugar1","Lugar1","Lugar2","Lugar3"};
    int tipo[]= {1,2,3,3};

    for(i=0; i<size; i++)
    {
        (*contadorID)++;
        array[i].idOrquesta= *contadorID;
        strcpy(array[i].varString,nombres[i]);
        strcpy(array[i].varLongString,lugar[i]);
        array[i].tipoOrquesta= tipo[i];
        array[i].isEmpty=0;
    }
}
