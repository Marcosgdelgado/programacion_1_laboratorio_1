...
void pub_printContratodiasmenoresA(Publicidad* listaPub,Pantalla* listaPan, int lenPan, int valor)
{
    int i;
    int posPan;
    float total;
    for ( i=0; i< lenPan;i++)
    {
        ...
        {
            posPan = pan_findPanById(listaPub,lenPan,listaPub[i].idPantalla);
            if (posPan>=0)
            {
