...
*
*/
int libro_alta(Libro array[],int idAutor, int size, int* contadorID)                          //cambiar libro
{
    int retorno=-1;
    int posicion;
               ...
               if(array!=NULL && size>=0)
                {
                    for(int i=0;i<size;i++)
                    {
                        if(array[i].isEmpty==1)
                            ...
                            printf("\n ID libro: %d\n Titulo: %s\n",
                                   array[i].idLibro,array[i].varLongString);
                    }
                    retorno=0;
                }
                                                                                                               //mensaje + cambiar campo varLongString
            printf("\n Posicion: %d\n ID: %d\n Titulo: %s\n ID AUTOR: %d\n",
                   posicion, array[posicion].idLibro,array[posicion].varLongString, array[posicion].idAutor);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            {       //copiar printf de alta
                printf("\n Posicion: %d\n ID: %d\n Titulo: %s",
                       posicion, array[posicion].idLibro,array[posicion].varLongString);
                utn_getChar("\nModificar:D S(salir)","\nError",'A','Z',1,&opcion);
                switch(opcion)
                {
 /*                   case 'A':
...
//            bufferFloat=array[i].varFloat;                            //cambiar campo varFloat
            ...
            j = i - 1;
           while ((j >= 0) && strcmp(bufferString,array[j].varLongString)<0)         //cambiar campo varString                 //Si tiene mas de un criterio se lo agrego, Ej. bufferInt<array[j].varInt
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID libro: %d\n Titulo: %s",
                       array[i].idLibro,array[i].varLongString);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
