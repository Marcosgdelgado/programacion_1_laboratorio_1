...
int autor_buscarString(Autor array[], int size, char* valorBuscado, int* indice)                    //cambiar autor
{
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(strcmp(array[i].varNombre,valorBuscado)==0)                                        //cambiar campo varNombre
            {
                *indice=i;
                retorno=0;
                break;
...
int autor_alta(Autor array[], int size, int* contadorID)                          //cambiar autor
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            utn_getName("\nALTA AUTOR\nNombre:\n ","\nError",1,TEXT_SIZE,1,array[posicion].varNombre);                      //mensaje + cambiar campo varNombre
            utn_getTexto("\nApellido:\n ","\nError",1,TEXT_SIZE,1,array[posicion].varApellido);                 //mensaje + cambiar campo varApellido
            printf("\n Posicion: %d\n ID: %d\n Nombre: %s\n apellido: %s\n",
                   posicion, array[posicion].idAutor,array[posicion].varNombre,array[posicion].varApellido);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
           ...
           // array[posicion].varInt=0;                                                               //cambiar campo varInt
            ...
            strcpy(array[posicion].varNombre,"");                                                   //cambiar campo varNombre
            strcpy(array[posicion].varApellido,"");                                               //cambiar campo varApellido
            retorno=0;
        }
    }
    return retorno;
        ...
        for(i=0;i<sizeArray;i++)
        {
            if(array[i].idAutor==valorBuscado)                                                        //cambiar si no se busca por ID
            {
               ...
               // array[i].varInt=0;                                                               //cambiar campo varInt
                ...
                strcpy(array[i].varNombre,"");                                                   //cambiar campo varNombre
                strcpy(array[i].varApellido,"");                                               //cambiar campo varApellido
            }
        }
        retorno=0;
    }
...
//    float bufferFloat;                                          //cambiar buffer varFloat
    ...
    if(array!=NULL && size>=0)
    {
        for (i = 1; i < size; i++)
        {
            strcpy(bufferString,array[i].varNombre);                      //cambiar campo varNombre
...
//            bufferInt=array[i].varInt;                                //cambiar campo varInt
 //           bufferFloat=array[i].varFloat;                            //cambiar campo varFloat
            strcpy(bufferLongString,array[i].varApellido);          //cambiar campo varApellido


            j = i - 1;
            while ((j >= 0) && strcmp(bufferString,array[j].varNombre)<0)         //cambiar campo varNombre                 //Si tiene mas de un criterio se lo agrego, Ej. bufferInt<array[j].varInt
            {                                                                                                               //buffer < campo ascendente   buffer > campo descendente
                strcpy(array[j + 1].varNombre,array[j].varNombre);          //cambiar campo varNombre
...
//               array[j + 1].varFloat=array[j].varFloat;                    //cambiar campo varFloat
                strcpy(array[j + 1].varApellido,array[j].varApellido);  //cambiar campo varApellido

                j--;
            }
            strcpy(array[j + 1].varNombre,bufferString);                     //cambiar campo varNombre
...
//            array[j + 1].varInt=bufferInt;                                                        //cambiar campo varInt
 //           array[j + 1].varFloat=bufferFloat;                                                    //cambiar campo varFloat
            strcpy(array[j + 1].varApellido,bufferLongString);                                  //cambiar campo varApellido
        }
        retorno=0;
    }
    return retorno;
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID autor: %d\n Nombre: %s\n Apellido: %s",
                       array[i].idAutor,array[i].varNombre,array[i].varApellido);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
