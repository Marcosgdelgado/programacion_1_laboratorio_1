...
int musico_alta(Musico array[], int size, int* contadorID, Orquesta arrayOrquesta[], int sizeOrq, Instrumento arrayInstrumento[], int sizeIns)                          //cambiar fantasma
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            utn_getSignedInt("\nIngrese edad: (18-90) ","\nError",1,sizeof(int),18,90,2,&array[posicion].varInt);
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            {       //copiar printf de alta
                printf("\n ID: %d\n varString: %s\n varLongString: %s",
                       posicion, array[posicion].idMusico,array[posicion].varString,array[posicion].varLongString);
        ...
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n NOMBRE: %s\n APELLIDO:\n %s\n EDAD: %d\n Nombre de instrumento: %d\n",
                       array[i].idMusico,array[i].varString,array[i].varLongString,array[i].varInt,array[i].idInstrumento);
                            switch(array[i].idInstrumento)
                            {
                            case 1:
                                printf("AVMAAA");
                                break;
