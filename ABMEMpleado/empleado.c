...
void mostrarEmpleado(eEmpleado unEmpleado, eSector sectores[], int tam)
{
    ...
    printf("%4d %10s   %2c    %6.2f %10s\n", unEmpleado.proximoId, unEmpleado.nombre, unEmpleado.sexo, unEmpleado.sueldo, descSector);
}
...
int buscarEmpleado(eEmpleado lista[], int tam, int proximoId)
{
    int indice = -1;
    ...
    for(int i=0; i < tam; i++)
    {
        ...
        if( lista[i].proximoId == proximoId && lista[i].isEmpty == 0)
        {
            indice = i;
            break;
        }
...
void altaEmpleado(eEmpleado lista[], int tam, eSector sectores[], int tamSector)
{
    ...
    else
    {
        ...
        existe = buscarEmpleado(lista, tam, proximoId);

        if( existe != -1)
        {

            printf("Ya existe un empleado con el proximoId %d\n", proximoId);
        ...
        else
        {
            ...
            nuevoEmpleado.proximoId = proximoId;
...
void bajaEmpleado(eEmpleado lista[], int tam, eSector sectores[], int tamSector)
{
    ...
    printf("Ingrese proximoId: ");
    scanf("%d", &proximoId);

    indice = buscarEmpleado(lista, tam, proximoId);

    if( indice == -1)
    {
        printf("No hay ningun empleado de proximoId %d\n\n", proximoId);
        system("pause");
    }
    else
    {
...
void ordenarEmpleadosXproximoId(eEmpleado lista[], int tam)
{
    ...
    for(int i=0; i<tam-1; i++)
    {
        for(int j= i+1; j < tam; j++)
        {
            ...
            if( lista[i].proximoId > lista[j].proximoId)
            {
                auxEmpleado = lista[i];
                lista[i] = lista[j];
                lista[j] = auxEmpleado;
